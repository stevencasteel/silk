...
export class AudioSystem implements ISystem, IUpdateable, IDisposable {
  ...
  public async init(): Promise<void> {
    ...
    this._tracker.add(
      this.context.broker.subscribe(GameEvent.PLAYER_STATE_CHANGE, (payload) => {
        if (payload.state !== "WALL_STICKING") {
          this.stopTetherStretch();
        }
        if (
  ...
  private startGeneralStretch(): void {
    if (this._isInitialized && this._generalStretchPlayer && this._generalStretchPlayer.loaded) {
      if (this._generalStretchPlayer.state !== "started") {
        this._generalStretchStartTime = Tone.now();
        const duration = this._generalStretchPlayer.buffer.duration || 1.0;
        const startOffset = this._generalStretchOffset % duration;
        this._generalStretchPlayer.start(undefined, startOffset);
      }
    }
  }

  private stopGeneralStretch(): void {
    if (this._generalStretchPlayer && this._generalStretchPlayer.state === "started") {
      const elapsed = Tone.now() - this._generalStretchStartTime;
      const duration = this._generalStretchPlayer.buffer.duration || 1.0;
      this._generalStretchOffset = (this._generalStretchOffset + elapsed) % duration;
      this._generalStretchPlayer.stop();
    }
  }
  ...
  public dispose(): void {
    ...
    if (this._generalStretchPlayer) {
      this._generalStretchPlayer.dispose();
      this._generalStretchPlayer = null;
    }
    if (this._tetherStretchPlayer) {
      this._tetherStretchPlayer.dispose();
      this._tetherStretchPlayer = null;
    }
