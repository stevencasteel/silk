  ...
  public update(dt: number): void {
    ...
    for (const projId of this.pool.getEntities()) {
      ...
      if (p.isTrappingPlayer) {
        ...
        const flashMax = 0.35;
        const flashAlpha = pTrav.webFlashTimer !== undefined ? Math.min(1.0, pTrav.webFlashTimer / flashMax) : 0.0;

        if (flashAlpha > 0) {
          const flashR = 0.01;
          const flashG = 0.01;
          const flashB = 0.01;
          rVal = rVal * (1.0 - flashAlpha) + flashR * flashAlpha;
          gVal = gVal * (1.0 - flashAlpha) + flashG * flashAlpha;
        ...
        if (flashAlpha > 0 && pTrav.lastEscapeDirection) {
          const stretchAmp = 0.55 * flashAlpha;
          const shiftAmp = 0.45 * flashAlpha;

          if (pTrav.lastEscapeDirection === "LEFT") {
            offsetX = -shiftAmp;
            scaleModX = stretchAmp;
            scaleModY = -stretchAmp * 0.35;
            scaleModZ = -stretchAmp * 0.35;
          } else if (pTrav.lastEscapeDirection === "RIGHT") {
            offsetX = shiftAmp;
            scaleModX = stretchAmp;
            scaleModY = -stretchAmp * 0.35;
            scaleModZ = -stretchAmp * 0.35;
          } else if (pTrav.lastEscapeDirection === "UP") {
            offsetY = shiftAmp;
            scaleModY = stretchAmp;
            scaleModX = -stretchAmp * 0.35;
            scaleModZ = -stretchAmp * 0.35;
          } else if (pTrav.lastEscapeDirection === "DOWN") {
            offsetY = -shiftAmp;
            scaleModY = stretchAmp;
            scaleModX = -stretchAmp * 0.35;
            scaleModZ = -stretchAmp * 0.35;
        ...
        if (pTrav.state === "WALL_STICKING") {
          ...
          targetScaleX = 0.28 + addedSize * 0.28 + scaleModX;
          targetScaleY = 2.0 + addedSize + scaleModY;
          targetScaleZ = 2.0 + addedSize + scaleModZ;

          const wallOffset = pTrav.wallDir * 0.25;
          trans.x = pTrans.x + wallOffset + offsetX;
          trans.y = pTrans.y + offsetY;
          trans.z = pTrans.z;
          trans.prevX = pTrans.prevX + wallOffset + offsetX;
          ...
          targetScaleY += scaleModY;
          targetScaleZ += scaleModZ;

          trans.x = pTrans.x + offsetX;
          trans.y = pTrans.y + offsetY;
          trans.z = pTrans.z;
          trans.prevX = pTrans.prevX + offsetX;
          trans.prevY = pTrans.prevY + offsetY;
          trans.prevZ = pTrans.prevZ;
          trans.qx = pTrans.qx;
          trans.qy = pTrans.qy;
