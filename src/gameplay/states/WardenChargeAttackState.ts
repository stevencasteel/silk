...
export class WardenChargeAttackState implements IWardenState {
  ...
  public update(ctx: AIContext, dt: number): WardenStateType | null {
    ...
    if (wTrav && wTrav.isWallClinging) {
      ctx.broker.publish(GameEvent.CAMERA_SHAKE_TRIGGERED, { amplitude: 0.8, duration: 0.5 });
      return "RECOVERY";
    }

    return ctx.ai.timeInState > 1.5 ? "RECOVERY" : null;
  }
}
