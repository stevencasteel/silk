...
export function decorateWeaverVisual(
  ...
  for (let i = 0; i < ridgeCount; i++) {
    ...
    const ridgeY = -radius * (0.1 + t * 0.6);
    const ridgeScale = Math.sin(t * Math.PI) * 0.9 + 0.1;
    const ridge = BABYLON.MeshBuilder.CreateTorus(
      `weaver_ridge_${i}`,
      {
        diameter: radius * 1.12 * ridgeScale,
        thickness: 0.08,
        tessellation: 12
      },
      scene
    );
    ridge.position.set(0, ridgeY, radius * 0.1);
    ridge.rotation.x = Math.PI / 2;
    ridge.scaling.set(1, 0.3, 1);
    ridge.material = carapaceMat;
    ridge.parent = abdomen;
  }
  ...
  const spinneretHeight = radius * 0.9;
  const spinneret = BABYLON.MeshBuilder.CreateCylinder(
    "weaver_spinneret",
    {
      height: spinneretHeight,
      diameterTop: radius * 0.42,
      diameterBottom: radius * 0.08,
      tessellation: 8
    },
    scene
  );
  spinneret.position.set(0, -radius * 1.05, -radius * 0.05);
  spinneret.rotation.x = -0.28;
  spinneret.material = legMat;
  spinneret.parent = wMesh;

  const spinneretGlow = BABYLON.MeshBuilder.CreateSphere(
    "weaver_spinneret_glow",
    { diameter: radius * 0.12 },
    scene
  );
  spinneretGlow.position.set(0, -spinneretHeight * 0.5, 0);
  spinneretGlow.material = eyeMat;
  spinneretGlow.parent = spinneret;
