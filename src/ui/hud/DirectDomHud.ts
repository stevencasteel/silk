...
export class DirectDomHud {
  ...
  private registerSubscribers(): void {
    this.unsubscribes.push(
      this.broker.subscribe(GameEvent.ROPE_TENSION_CHANGE, (payload) => {
        this.updateTensionDisplay(payload.tension);
      })
    );

    this.unsubscribes.push(
      this.broker.subscribe(GameEvent.PLAYER_VELOCITY_CHANGED, (payload) => {
        this.updateVelocityDisplay(payload.velocity, payload.maxVelocity);
      })
    );

    this.unsubscribes.push(
      this.broker.subscribe(GameEvent.PLAYER_HEALTH_CHANGED, (payload) => {
        this.updateHealthDisplay(payload.hp, payload.maxHp);
      })
    );

    this.unsubscribes.push(
      this.broker.subscribe(GameEvent.WARDEN_STATE_CHANGE, (payload) => {
        this.updateWardenStateDisplay(payload.state, payload.hue);
      })
    );
  }

  public updateTensionDisplay(value: number): void {
    ...
    if (this.tensionBar) {
      ...
      if (clamped > 0.8) {
        this.tensionBar.style.backgroundColor = "var(--signal-red, #ef4444)";
      } else if (clamped > 0.5) {
        this.tensionBar.style.backgroundColor = "var(--signal-yellow, #eab308)";
      } else {
        this.tensionBar.style.backgroundColor = "var(--signal-green, #22c55e)";
      }
    }
    if (this.tensionText) {
      this.tensionText.textContent = `TENSION: ${percentage}`;
    }
  }

  public updateVelocityDisplay(velocity: number, maxVelocity: number): void {
    ...
    if (this.speedText) {
      this.speedText.textContent = `VELOCITY: ${velocity.toFixed(0)} / ${maxVelocity.toFixed(0)} m/s`;
    }
  }

  public updateHealthDisplay(hp: number, maxHp: number): void {
    if (this.hpValue) {
      this.hpValue.textContent = `HP: ${hp} / ${maxHp}`;
    }
    if (this.hpText) {
      const hpPct = ((hp / maxHp) * 100).toFixed(0) + "%";
      this.hpText.style.width = hpPct;
      if (hp <= 1) {
        this.hpText.style.backgroundColor = "var(--signal-red, #ef4444)";
      } else {
        this.hpText.style.backgroundColor = "var(--signal-green, #22c55e)";
      }
    }
  }

  public updateWardenStateDisplay(state: string, hue: string): void {
